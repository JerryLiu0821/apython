...
class testUpgrade(unittest.TestCase):
    ...
    def connect(self):
        ...
        for i in range(1, 6):
            print self.id
            result = os.popen('adb connect ' + self.id).readlines()
            print result
    ...
    def testSTAB_USB_UPGRADE_TO_NEW(self):
        ...
        #self.jar = "UiAutomator.jar"
        self.case = "com.letv.system.TestUpgrade#testUpgrade"
        try:
    ...
    def testCancelDownload(self):
        '''usb升级下载时，取消下载升级包|usb升级下载时，取消下载升级包：1. 进入离线升级，复制下载升级包 2. 下载过程中取消下载 Fail项：1.无法停止下载 2.无法返回到下载时的前一界面'''
        self.jar = "UiAutomator.jar"
        #self.jar = "UiAutomator.jar"
        self.case = "com.letv.system.TestUpgrade#testCancelUpgrade"
        try:
            #self.a.device.sh("rm -rf %s/*" %self.path)
            ua = Stability.UiAutomator(self.id, self.jar, self.case)
            result, info = ua.runtest()
    ...
    def _testSTAB_USB_UPGRADE_TO_NEW(self):
        """U盘升级 | U盘升级"""
        self.a.device.sh("rm /mnt/usb/sda1/update.zip")
        print "copy the file"
        ...
        self.jar = "UiAutomator.jar"
        #self.jar = "UiAutomator.jar"
        self.case = "com.letv.system.TestUpgrade"
        try:
            #self.a.device.sh("rm -rf %s/*" %self.path)
            ua = Stability.UiAutomator(self.id, self.jar, self.case)
            result, info = ua.runtest()
