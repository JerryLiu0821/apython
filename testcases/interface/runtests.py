...
""" % ('%s', self.current['name'])
	...
	def getVersionFromTraceLog(self, path):
		version = ''
		pattern = re.compile(r'.*\[ro\.letv\.release\.date\].*\[(.*)\]')
		fp = open(path, "r")
		try:
			lines = fp.readlines()
			fp.close()
	...
	def generate_index(self, result=None):
		...
		try:
			...
			if result:
				...
				version_value = self.getVersionFromTraceLog(os.path.join(android.log.report_directory(), 'logs/00001/trace_log.1.txt'))
				pass_value = self.passed
				fail_value = failNumber if failNumber != -1 else 0
				error_value = errorNumber if errorNumber != -1 else 0
				...
				output('<font size=12>自动冒烟测试用例结果：</font>')
				output('Version: %s dailybuild'%version_value)
