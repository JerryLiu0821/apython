...
class testLocalPlayByTypes(unittest.TestCase):
    ...
    def testPlayTheFolderVideos(self):
        "播放所有格式的视频文件|操作步骤:;1. play video in mediaContent folder in storage"
        self.jar = "UiAutomator.jar"
        #self.jar = "UiAutomator.jar"
        self.case = "com.letv.media.LocalPlay"
        self.arg = "count"
        self.argvalue = 1
        try:
        ...
        try :
            #start filemanager
            self.a.device.sh(" am start -a android.intent.action.MAIN -c android.intent.category.LAUNCHER -f 0x10200000 -n com.stv.filemanager/.FileExplorerTabActivity")
            time.sleep(4)
            self.playFiles('/mnt/usb/sda1/mediaContent', 'f')
        except Exception, e :
            self.a.log.debug("", "\n testPlayTheFolderVideos")
    ...
    def playFiles(self, folder_path, Media_Type):
        ...
        for j in range(1, self.outloop+1):
            for i in range (0,self.loop):
                ...
                cmd = "am start -a android.intent.action.VIEW -d \'%s\' -n com.stv.videoplayer/.MainActivity" %self.play_urls[i]
