    ...
    def generate_index(self, result=None):
        ...
        try:
            ...
            if result:
                ...
                output('<font size=6>Version: %s dailybuild </font>'%version_value)
                output('<font size=6>Product: %s </font>'%product_value)
                ...
                output('</table>')
                output('')
                output('procrank Pss info: <a href="%s">TOTAL</a> <a href="%s">TOP_10_PROCESS</a>' %("./total_pss.svg", "./top_10_pss.svg"))
